...
namespace WinsockPacketEditor
{
    ...
    /// 都自带中文原文，<see cref="CoreL10n"/> 查不到时用的就是那一份。
    /// 再抄一份简体表进来，只会多出一处要同步的地方。
    /// 所以简体没有表，其余六种各一份。
    ///
    /// 【回退链分两种】
    ///   · 一般语言：当前 → 英文 → null（CoreL10n 再回退到调用点的简体兜底）。
    ///     中间这一档是给「新加了键、译文还没跟上」那半天用的，那时露出英文比露出键名强。
    ///   · <b>繁体中文：当前 → null，跳过英文。</b>缺键时落到调用点的简体兜底 ——
    ///     对繁体读者来说简体比英文近得多，字形差异远小于换一门语言。
    ///
    /// 【为什么表是懒加载的】六份表各 335 条（2026-09-11 删 WinForms 之后收到 324 条，之后又加回十来条；原来各 1100 条）。
    ...
    public static class L10n
    {
        ...
        /// <summary>
        /// 取一条译文。取不到返回 null —— 调用方（CoreL10n）据此回退到中文兜底。
        /// </summary>
