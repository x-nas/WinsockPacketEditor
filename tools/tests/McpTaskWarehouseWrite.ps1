...
try {
    ...
    $send = Call 2 'wpe_task_create' @{ kind='send'; idempotencyKey=Key }; $robot = Call 3 'wpe_task_create' @{ kind='robot'; idempotencyKey=Key }; $warehouse = Call 4 'wpe_task_create' @{ kind='warehouse'; idempotencyKey=Key }
    ...
    $null = Call 5 'wpe_task_update' @{ kind='send'; id=$send.id; name='MCP write-test send'; useSystemSocket=$false; loopCount=1; loopInterval=0; notes='temporary MCP regression task'; idempotencyKey=Key }
    $null = Call 6 'wpe_task_update' @{ kind='robot'; id=$robot.id; name='MCP write-test robot'; idempotencyKey=Key }
    $null = Call 7 'wpe_task_update' @{ kind='warehouse'; id=$warehouse.id; name='MCP write-test warehouse'; idempotencyKey=Key }
    ...
    $copies = @(); foreach ($item in @(@{ kind='send'; id=$send.id; tool='wpe_sends_list' }, @{ kind='robot'; id=$robot.id; tool='wpe_robots_list' }, @{ kind='warehouse'; id=$warehouse.id; tool='wpe_warehouse_list' })) { $before = @(Call (10 + $copies.Count * 2) $item.tool @{ limit=200 }).rows.Id; $copy = Call (11 + $copies.Count * 2) 'wpe_tasks_copy' @{ kind=$item.kind; ids=@($item.id); idempotencyKey=Key }; $after = @(Call (12 + $copies.Count * 2) $item.tool @{ limit=200 }).rows.Id; $copyId = @($after | Where-Object { $_ -notin $before }) | Select-Object -First 1; if (-not $copy.changed -or [string]::IsNullOrEmpty($copyId)) { throw "Copy failed for $($item.kind)." }; $copies += @{ kind=$item.kind; id=$copyId } }
    $null = Call 20 'wpe_tasks_move' @{ kind='send'; ids=@($send.id); direction='top'; idempotencyKey=Key }
    $null = Call 21 'wpe_tasks_move' @{ kind='robot'; ids=@($robot.id); direction='top'; idempotencyKey=Key }
    $null = Call 22 'wpe_tasks_move' @{ kind='warehouse'; ids=@($warehouse.id); direction='top'; idempotencyKey=Key }
    $null = Call 23 'wpe_tasks_delete' @{ kind='send'; ids=@($send.id, $copies[0].id); idempotencyKey=Key }
    $null = Call 24 'wpe_tasks_delete' @{ kind='robot'; ids=@($robot.id, $copies[1].id); idempotencyKey=Key }
    $null = Call 25 'wpe_tasks_delete' @{ kind='warehouse'; ids=@($warehouse.id, $copies[2].id); idempotencyKey=Key }
