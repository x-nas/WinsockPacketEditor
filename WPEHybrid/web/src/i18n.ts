...
// 于是 UI.T 也换语言，不会出现「界面英文、弹窗中文」。
//
// 【字典写成一键两值，不是两份平行字典】
// 官网 cyber.js 的 INDEX_ZH / INDEX_EN 是两份平行数组，加条目要改两处、
// 顺序错位还不报错。这里把 zh / en 贴在同一个键下，漏译在类型上就是缺字段。
//
...
const DICT = {
  ...
  // 与官网语言 chip 的 title 逐字一致（cyber.js 的 langSwitch）
  'win.lang': { zh: '切换到 English', en: '切换到中文 / Switch to Chinese' },
  // chip 上显示的是<b>当前</b>语言而不是将要切到的语言 —— 同样照官网的 langLabel
  'win.langLabel': { zh: 'CN', en: 'EN' },
  'win.nohost': {
...
export function t(key: Key): string {
  const e = DICT[key] as Entry | undefined
  if (!e) {
    console.warn('[i18n] 未知文案键:', key)
    return key
  }
  return lang.value === 'en' ? e.en : e.zh
}

/** C# 的 "zh-CN" / "en-US" → 这里的两态。 */
export function normalize(code: string | undefined | null): Lang {
  return (code || '').toLowerCase().startsWith('en') ? 'en' : 'zh'
}
...
export async function toggleLang(): Promise<void> {
  const next: Lang = lang.value === 'en' ? 'zh' : 'en'
  lang.value = next
  applyDocumentLang()

  try {
    await call('setLanguage', { language: next === 'en' ? 'en-US' : 'zh-CN' })
  } catch (e) {
