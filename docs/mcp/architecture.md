...
MCP client (Codex / Claude / compatible client)
        | stdio, JSON-RPC
        v
WPEMcpServer.exe (.NET 10, self-contained single-file, asInvoker)
...
- Implements only MCP transport, discovery, schemas, pagination and MCP-shaped errors. The launcher publishes this single file at `C:\WPE64DB\McpServer\WPEMcpServer.exe`.
...
3. The connector requires exactly one running WPE instance and opens its pipe under the same Windows user. If a write call omits an empty `idempotencyKey`, the Sidecar supplies a UUID; explicitly supplied keys are retained for deterministic retries.
