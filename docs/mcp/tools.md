...
| `wpe_start_mode_select` | `start.mode.select` | `write.startup.mode` | Selects proxy or inject mode only while WPE is on the start page; does not start proxy, select a target, or inject automatically |
